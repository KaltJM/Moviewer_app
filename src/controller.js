...
function getMovies (req,res) {
    pool.query(query.getMovies,function (error,results) { 
        ...
        else if (req.query.genre){ //Filtro para la búsqueda específica
            var movies = [];
            for(let i = 0; i < results.rows.length; i++){ //Recorrido de lista de películas existentes en la DB

                genero_movie = results.rows[i].genero; //Género de pelicula i en la lista
                genero_peticion = req.query.genre; //Género consultado

                if (results.rows[i].genero){ //Filtro de objetos en  lista de películas (revisar)
...
function insertMovie (req,res) {
    ...
    pool.query(query.insertMovie,
        [nombre,fecha_de_lanzamiento,costo_de_creacion,genero], //Parámetros de consulta
        function (error) {//Uso de consulta para guardar película en DB
        if (error){
            throw error;
        }
        else{//Código para obtener la película una vez guardada en la DB (con id)
...
function updateMovie (req,res) {
    const {nombre,fecha_de_lanzamiento,costo_de_creacion,genero} = req.body; //Parámetros del request
    pool.query(query.updateMovie,
        [nombre,fecha_de_lanzamiento,costo_de_creacion,genero,req.params.id], //Parámetros de consulta (ojo id)
        function (error) {
        if (error){
            throw error;
        }
...
function checkMovieExists (req,res) {
    switch(req.method){ //Dependiendo de operación a realizar
        case 'POST':
            pool.query(query.checkMovieExistsbyNombre,[req.body.nombre],function(error,results){ //Verificar que existe (nombre)
                if (error){
                    throw error;
                }
                else{ 
                    if (results.rows.length){ //Filtro para saber si existe
        ...
        case 'DELETE':
            pool.query(query.checkMovieExistsbyid,[req.params.id],function(error,results){ //Verificar que existe (id)
                if (error){
                    throw error;
                }
                else{
                    if (results.rows.length){ //Se elimina la película
                        deleteMovie(req,res);
                    }
                    else{
        ...
        case 'PUT':
            pool.query(query.checkMovieExistsbyid,[req.params.id],function(error,results){ //Verificar que existe (id)
                if (error){
                    ...
                    if (results.rows.length){ //Se actualiza la película
                        updateMovie(req,res);
                    }
                    else{
